...
String hId=request.getParameter("id").trim();
ArrayList<Hall> alHall=m.getHallDetails(hId);
if(alHall.size()==0)
{
out.println("<h1>No slot is allocated in this room</h1>");
return ;
}
HashMap<Integer,Hall[]> table=new HashMap<>();
int curDayId=0;
	...
	<div id="table" class="time-table none">
        ...
        %>
			...
			<tbody>
				<%
      int dayId = 1;
      Hall arr[] = new Hall[16];
      int slotId = 1;
      Arrays.fill(arr, null);
      for (Hall hall : alHall) {
      	while (hall.getDayId()>dayId) {
      	...
      	if (hall.getDayId() == dayId) {
      
      		while (slotId < hall.getSlot().getId()) {
      	arr[slotId++] = null;
      
      		}
      		
      		if (slotId == hall.getSlot().getId()) {
     ...
     // System.out.println("158"+table.get(4)[3].getHallCourse().getCourseName());
     // System.out.println("158"+table.get(4)[2].getHallCourse().getCourseName());


      
 /*      for(Hall iop:table.get(5))
      {
    	  System.out.println("160: "+iop.getHallCourse());
      }
       */
      // displaying Table:
      ...
      for (int i = 1; i <= 5; i++) {
      	...
      	out.println("<td class=\"left\" >Theory</td>");
      		...
      		for (int di = 1; di <= 15; di++) {
      			...
      			if (table.get(i)[di] != null && !table.get(i)[di].getIsLab())
      			{
      				out.println("<td  class=\"available\">" + table.get(i)[di].getHallCourse().getCourseName() +"<br>"+table.get(i)[di].getHallId() + "</td>");
      			}
